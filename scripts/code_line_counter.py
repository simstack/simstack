...
def count_lines_of_code(directory: str, extensions: Set[str] = None, excluded_patterns: List[str] = None) -> Dict[str, Dict[str, int]]:
    """
    ...
    for ext in extensions:
        results['by_extension'][ext] = {'total': 0, 'code': 0, 'blank': 0, 'comments': 0, 'file_count': 0}
    
    # Walk through directory recursively
    for file_path in directory_path.rglob('*'):
        if file_path.is_file() and file_path.suffix in extensions:
            # Check if file should be excluded
            if should_exclude_path(file_path.relative_to(directory_path), excluded_patterns):
...
def print_results(results: Dict[str, Dict[str, int]], show_files: bool = False):
    ...
    if show_files:
        ...
        for file_path, stats in results['files'].items():
            print(f"{file_path}:")
            print(f"  Total: {stats['total']:,}, Code: {stats['code']:,}, "
...
def main():
    ...
    try:
        ...
        if excluded_patterns:
            print(f"Excluding patterns: {', '.join(excluded_patterns[:5])}" + 
                  (f" (and {len(excluded_patterns)-5} more)" if len(excluded_patterns) > 5 else ""))
        print()
