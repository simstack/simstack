...
def test_filestack_hash_different_content_same_name():
    ...
    try:
        ...
        assert hash1 != hash2, f"Hashes should be different for different content, but got: {hash1} == {hash2}"
...
def test_filestack_hash_same_content_same_hash():
    ...
    try:
        ...
        assert hash1 == hash2, f"Identical content should produce same hash: {hash1} vs {hash2}"
