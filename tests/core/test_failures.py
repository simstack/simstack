...
def hello_world_file():
    ...
    with open(hello_world_file_path, "w") as f:
        f.write("Hello World!")
    hello_world_file_stack = FileStack.from_local_file(hello_world_file_path, in_memory=True)
    yield hello_world_file_stack
    os.unlink(hello_world_file_path)

@node
def failing_node_with_runner(info_file: FileStack, hello_world_file: FileStack, **kwargs) -> IntData:
...
async def test_failing_node_with_runner(info_file, hello_world_file):
    ...
    node_registry = await context.db.find_one(NodeRegistry,NodeRegistry.id == ObjectId(task_id))
