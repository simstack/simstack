...
from src.simstack.util.minimal_route_finder import find_shortest_route
...
def mock_context(setup_test_env):
    ...
    with patch("src.simstack.models.files.context", 
               MockContext(setup_test_env["workdir"])):
        yield

@pytest.fixture
def mock_job_submission():
    """Mock the job submission and status check functions."""
    with patch("src.simstack.models.files.submit_copy_job") as mock_submit, \
...
@pytest.mark.skip(reason="file copy tests must be fixed")
def test_filestack_get_remote(setup_test_env, mock_context, mock_job_submission, mock_file_creation):
    ...
    with patch("src.simstack.models.files.find_shortest_route") as mock_route_finder:
        ...
        def create_file_after_status(*args, **kwargs):
            if mock_status.call_count == 2:  # After the second status check (when "RUNNING")
                expected_output_path = setup_test_env["output_dir"] / "remote_file.txt"
                mock_file_creation(expected_output_path)
            return mock_status.side_effect.pop(0) if mock_status.side_effect else "COMPLETED"
...
@pytest.mark.skip(reason="file copy tests must be fixed")
def test_filestack_get_multi_remote_first_fails(setup_test_env, mock_context, mock_job_submission, mock_file_creation):
    ...
    with patch("src.simstack.models.files.find_shortest_route") as mock_route_finder:
        ...
        def submit_side_effect(*args, **kwargs):
            ...
            print(f"Mock submit called with source: {source_node}, target_path: {target_path}")
        ...
        assert mock_submit.call_count == 2, "Both remote instances should have been tried"
        assert "remote1" in mock_submit.call_args_list[0][1]["source_node"], "First attempt should be remote1"
        assert "remote2" in mock_submit.call_args_list[1][1]["source_node"], "Second attempt should be remote2"

@pytest.mark.skip(reason="file copy tests must be fixed")
def test_filestack_get_no_valid_instances(setup_test_env, mock_context, mock_job_submission):
