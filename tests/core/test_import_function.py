...
async def setup_pickled_function(initialized_context):
    ...
    function_pickle = FunctionPickle(
        name="another_node_for_testing",
        module_path="tests.core.test_import_function"
    )
...
async def test_import_function_from_pickle(setup_pickled_function):
    ...
    func = await import_function("tests.core.test_import_function.another_node_for_testing")
...
async def test_import_function_nonexistent():
    ...
    with pytest.raises(LookupError, match="Function tests.core.test_import_function.nonexistent_function not found in the NodeModel Table"):
