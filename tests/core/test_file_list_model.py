...
class TestFileListModel:
    ...
    @pytest.mark.asyncio
    async def test_save_and_load_file_list_model(self, initialized_context, sample_file_stacks):
    ...
    @pytest.mark.asyncio
    async def test_update_file_list_model_in_database(self, initialized_context, sample_file_stack):
    ...
    @pytest.mark.asyncio
    async def test_delete_file_list_model(self, initialized_context, sample_file_stacks):
        ...
        # Verify it exists
        loaded_model = await context.db.find_one(FileListModel, FileListModel.id == saved_id)
        assert loaded_model is not None
        ...
        # Verify it's gone
        deleted_model = await context.db.find_one(FileListModel, FileListModel.id == saved_id)
        assert deleted_model is None
    
    @pytest.mark.asyncio
    async def test_find_multiple_file_list_models(self, initialized_context, sample_file_stacks):
    ...
    async def test_file_list_model_with_file_operations(self, initialized_context):
        ...
        try:
            ...
            loaded_model = await context.db.find_one(
                FileListModel,
                FileListModel.id == saved_model.id
            )
...
def create_test_file_stack(name: str, size: int = 100, in_memory: bool = True) -> FileStack:
