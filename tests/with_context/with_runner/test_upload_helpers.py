...
from simstack.methods.upload_helpers import archive_upload, file_list_upload_test, file_list_upload_all
...
async def test_archive_upload_zip(temp_dir):
    ...
    assert len(result) == 1
    extracted_file_stack = result.get(0)
...
async def test_archive_upload_regular_file(temp_dir):
    ...
    assert isinstance(result, FileListModel)
    assert len(result) == 1
    file_stack_result = result.get(0)
