...
async def test_test_file_model_save_and_retrieve(initialized_context, sample_file_stacks):
    """Test saving and retrieving TestFileModel with embedded FileList"""
    ...
    # 3. Create TestFileModel and embed FileList
    model = TestFileModel(file_list=file_list, name="Test Model")
    
    # 4. Save TestFileModel
    saved_model = await context.db.save(model)
    assert saved_model.id is not None
    assert len(saved_model.file_list) == 2
    
    # 5. Retrieve TestFileModel from database
    retrieved_model = await context.db.find_one(TestFileModel, TestFileModel.id == saved_model.id)
...
async def test_test_file_model_empty_list(initialized_context):
    """Test TestFileModel with an empty FileList"""
    model = TestFileModel(name="Empty Model")
    saved_model = await context.db.save(model)
