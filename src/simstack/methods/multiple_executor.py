...
@node
def multiple_executor_adder(arg1: BinaryOperationInput, arg2: BinaryOperationInput) -> BinaryOperationInput:
    return BinaryOperationInput(
        arg1=arg1.arg1 + arg2.arg1,
        arg2=arg1.arg2 + arg2.arg2
    )
...
async def get_mappings(func: Callable[[Model], Model]):
    ...
    result_model = await context.db.find_one(ModelMapping, ModelMapping.name == return_type.__name__)
...
class MultipleExecutorInput(Model, Generic[T]):
    ...
    def from_function(cls, func: Callable[[T], R]):
        ...
        return cls(function_mapping=function_mapping,
                   input_mappings=input_mappings,
                   result_mapping=result_mapping)

    async def input_generator(self) -> AsyncGenerator[List[Model], None]:
        """Deserialize the input models"""
        input_model_classes = [await import_class(input_mapping) for input_mapping in self.input_mappings]
        for input_item in self.inputs_data:
            input_models = [await context.db.find_one(input_model_class, input_model_class.id == model_id)
            ...
            if any(model is None for model in input_models):
                raise ValueError(f"One or more input models not found in database for input IDs: {input_item}")
            yield input_models
    ...
    async def execute(self, **kwargs) -> 'MultipleExecutorOutput':
        ...
        if pending:
            ...
            for task in pending:
                logger.error(f"Task at index {task.index} didn't complete within timeout")
...
class MultipleExecutorOutput(Model, Generic[T, R]):
    ...
    async def result_generator(self) -> AsyncGenerator[Union[R, None], None]:
        ...
        for result_id in self.result_ids:
            if result_id is not None:
                result_model = await context.db.find_one(result_model_class, result_model_class.id == result_id)
    ...
    async def make_table(self):
        ...
        return {
            'rowData': merged_entries,
            'columnDefs': merged_columns
        }


@node
async def multiple_executor(executor: MultipleExecutorInput, **kwargs) -> MultipleExecutorOutput:
...
async def main():
    ...
    async for item1, item2 in async_zip(executor_input.input_generator(), executor_output.result_generator()):
