...
async def run_docker(registry_entry: NodeRegistry):
    ...
    cmd = [
        ...
        "-d",
        "-e",
        f"SIMSTACK_DB_DATABASE={context.config.db_name}",
        "-e",
        f"SIMSTACK_DB_TEST_DATABaASE={context.config.db_name}",
        "-e",
        f"SIMSTACK_DB_CONNECTION_STRING={context.config.connection_string}",
        "-v",
        f"{workdir}:/root/simstack",
        image,
        "uv",
        "run",
        "run_node",
    ...
    if stdout:
        ...
        logger.info(
            "Spawned docker container for task_id=%s: %s", registry_entry.id, stdout
        )
    if stderr:
        # Some Docker setups warn on stderr even on success
        logger.warning(
            "docker run stderr for task_id=%s: %s", registry_entry.id, stderr
        )
