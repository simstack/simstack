...
class GitUvUpdateService(BaseService):
    ...
    def __init__(self, manager, interval):
        ...
        self.project_dir = context.config.project_dir.resolve(strict=True)
        self.uv_lock_path = context.config.working_dir / "uv.lock"
        self.pid_file = context.config.working_dir / "runner.pid"
    ...
    async def trigger_restart(self):
        ...
        if platform.system() == "Windows":
            ...
            # This cmd string kills the PID, waits 2 seconds, then starts python
            cmd = f"taskkill /F /PID {current_pid} && timeout /t 2 && {' '.join(args)}"
...
class GitRestartService(BaseService):
    ...
    async def execute(self):
        if not context.config.python_path:
            return

        base_path = Path(context.config.project_path).resolve(strict=True)
        cmd = base_path / "scripts" / "check_runner.sh"
        log_file_path = Path(context.config.working_dir) / "cron_log.log"

        if not cmd.exists():
            logger.warning(f"Restart script not found at {cmd}")
            return

        logger.info(f"Executing external restart check: {cmd}, logging to {log_file_path}")

        try:
            # Open the log file in append mode.
            # We don't use 'with' here because we want the child to inherit the handle.
            ...
            # The file handle is inherited by the child. We can close it in the parent
            # immediately after fork; the child keeps its own copy of the descriptor.
            log_file.close()
            
        except Exception as e:
            logger.error(f"Failed to spawn restart script: {e}")
