...
class RunnerManager:
    ...
    async def run_nodes_for_resource(self,
        ...
        async def run_node_with_semaphore(registry_entry_arg):
            async with semaphore:
                return await self.run_node(registry_entry_arg)
            ...
            while True:
                try:
                    ...
                    for path in context.config.python_path:
                        ...
                        if stop_file.exists():
                            await self.write_resource_event(RunnerEventEnum.SHUTDOWN,message="STOP file found at STOP")
                    ...
                    running_jobs = await context.db.engine.find(
                        ...
                        & (NodeRegistry.parameters.resource == self.resource),
                    )
