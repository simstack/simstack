...
class DataSetSection(EmbeddedModel):
    ...
    """
    model_types: List[str] = Field(default_factory=list)  # Class names of models in each tuple
    data: List[List[ObjectId]] = Field(default_factory=list)  # List of tuples (as lists of ObjectIds)
    ...
    def add_model_group(self, models: Union[Model,Tuple[Model, ...]]) -> None:
        ...
        if not self.model_types:
            ...
            if model_names != self.model_types:
                raise ValueError(f"Model types {model_names} don't match section's expected types {self.model_types}")
    ...
    async def get_model_group(self, index: int) -> Tuple[Model, ...]:
        ...
        if index >= len(self.data):
            raise IndexError(f"Index {index} out of range for section with {len(self.data)} model groups")
        ...
        for model_type, model_id in zip(self.model_types, model_ids):
            model_class = await import_class_by_name(model_type)
            model_instance = await context.db.find_one(model_class, model_class.id == model_id)
            if model_instance is None:
                raise ValueError(f"Model of type {model_type} with id {model_id} not found")
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[Tuple[Model, ...], List[Tuple[Model, ...]]]:
        """
    ...
    async def index(self, models: Tuple[Model, ...], start: int = 0, stop: int = None) -> int:
        """
    ...
    def __repr__(self) -> str:
        ...
        return f"DataSetSection(model_types={self.model_types}, length={len(self.data)})"
...
class DataSet(Model):
    ...
    def collect_structure(self) -> Dict[str, List[str]]:
        ...
        """
        return {section_name: section.model_types if len(section)>0 else None for section_name, section in self.sections.items()}
    ...
    def update(self, other: Union[Dict[str, DataSetSection], 'DataSet'] = None, **kwargs) -> None:
