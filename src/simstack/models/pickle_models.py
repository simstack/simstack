...
class FunctionPickle(_BytesB64Mixin, Model):
    ...
    def _is_problematic_object(self, obj) -> bool:
        ...
        try:
            ...
            problematic_types = {
                '_OverlappedFuture', 'ProactorEventLoop', 'WindowsProactorEventLoopPolicy',
                '_ProactorBasePipeTransport', '_ProactorSocketTransport', 'Handle',
                '_WindowsSelectorEventLoop', '_ProactorReadPipeTransport'
            }
    ...
    def _clean_globals(self, func_globals: dict) -> dict:
        ...
        for key, value in func_globals.items():
            try:
                ...
                skip_names = {
                    '_OverlappedFuture', '_overlapped', '_winapi'
                }
                ...
                if key == '__builtins__':
                    if isinstance(value, dict):
                        ...
                        cleaned_builtins = {k: v for k, v in value.items() 
                                          if k in essential_builtins and not self._is_problematic_object(v)}
                ...
                if cleaned_value is not None:
                    ...
                    try:
                        ...
                        logger.debug(f"Skipping '{key}' after cleaning failed pickle test: {e}")
                else:
    ...
    def store_function(self, func: Callable) -> None:
        ...
        try:
            ...
            logger.warning(f"Function {self.name} will use regular import instead of pickle")
    ...
    def load_function(self) -> Callable:
        ...
        logger.debug("Loaded function %s (%s)", self.name, getattr(func, '__name__', 'unnamed'))
        return func
