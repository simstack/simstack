...
def parse_spec(cmd_file: Path) -> Tuple[Optional[str], Dict[str, str], Optional[str], Optional[int]]:
...
def run_command(command: str, cwd: Optional[str], env: Dict[str, str], timeout: Optional[int],
                out_file: Path, err_file: Path) -> int:
    ...
    try:
        ...
        if claimed_by_lock:
            try:
                ...
                print(f"[watcher] Warning: failed to remove ready {ready_file}: {e}", file=sys.stderr)
            try:
                if lock_file.exists():
                    lock_file.unlink()
            except Exception as e:
                print(f"[watcher] Warning: failed to remove lock {lock_file}: {e}", file=sys.stderr)
        else:
            try:
                if claim_file.exists():
                    claim_file.unlink()
            except Exception as e:
                print(f"[watcher] Warning: failed to remove claim {claim_file}: {e}", file=sys.stderr)

def try_claim(ready_file: Path, retries: int = 5, backoff: float = 0.05) -> Optional[Path]:
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Concurrent file-queue watcher with robust claiming")
    parser.add_argument("--queue-dir", type=Path, required=False, default=Path.cwd() / "queue")
    parser.add_argument("--poll-interval", type=float, default=1.0)
    parser.add_argument("--workers", type=int, default=min(4, (os.cpu_count() or 2)))
    parser.add_argument("--use-srun", action="store_true", help="(deprecated; wrapper removed for simplicity)")
    parser.add_argument("--srun-args", type=str, default="")
    parser.add_argument("--once", action="store_true", help="Process current claims and exit")
