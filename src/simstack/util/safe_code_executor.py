...
def safe_code_executor(code_string: str, artifact_arguments: ArtifactArguments, timeout: int = 30) -> Dict:
    ...
    for name in ['abs', 'all', 'any', 'bool', 'dict', 'enumerate', 'filter', 
    ...
    from examples.science.electronic_structure.spectra.plot_spectra import make_multi_line_chart
    ...
    execution_result = {'completed': False, 'exception': None, 'local_vars': {}, 'function_result': None}
    ...
    try:
        ...
        if execution_result['function_result'] is not None:
            ...
            if 'result' in local_vars:
                ...
                result['result'] = {k: v for k, v in local_vars.items() if not k.startswith('_')}
