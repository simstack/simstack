...
async def initialize_resource_from_db(
    resource_str: str, db: Database, workdir_self: Path
) -> ResourceDefinition:
    resource_records = await db.find(ResourceDefinition)
    if resource_records is not None:
        ...
        if resource_str not in allowed_resources_list:
            raise ValueError(
                f"Resource {resource_str} not found in the list of allowed resources"
            )
        ...
        if resource_str == "self":
            ...
            resource_definition = next(
                (r for r in resource_records if r.resource_str == resource_str), None
            )
        ...
        for resource_record in resource_records:
            route_table.add_route_set(
                resource_record.resource_str, resource_record.routes
            )
